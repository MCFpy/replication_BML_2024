...
# Integrate code
PATH = "F:/fmascolo/ohe/data/"

# This is the the only df with the cleaning applied for the pseudo dates
#df = pd.read_csv(PATH + "ohe_test_mcf.csv")

df = pd.read_csv(PATH + "ohe_data1.csv")

df.doc_any_12m.value_counts(dropna=False)

df = df[~df['doc_any_12m'].isna()]

df.doc_any_12m = np.where(df.rx_any_12m == "Yes", 1, 0)

df.isna().sum()

df = df[~df['female_list'].isna()]


df.birthyear_list.describe()
...
APPLIC_PATH = "F:/fmascolo/ohe/data/"
DATPATH = APPLIC_PATH + '/data'
# ALLDATA = 'data_y_d_x_4000.csv'
# Training data must contain outcome, treatment and features.
OUTPUT_PATH = "F:/fmascolo/ohe/"


# ------------------ Reset some parameters  -------------------
VAR_D_NAME = 'treatment'   # Name of treatment variable
VAR_Y_NAME = 'doc_any_12m'         # Name of outcome variable


VAR_POLSCORE_NAME = ('doc_any_12m_LC0_un_lc_pot',
                     'doc_any_12m_LC1_un_lc_pot',)


# ordered variables  
var_x_name_ord = ['age', 'numhh_list', 'draw', 'birthyear_list',
                  'have_phone_list','pobox_list', 'english_list',
                  'female_list', 'zip_msa']
...
# Gates variables
var_z_name_list = []
var_z_name_ord = ['numhh_list','age', 'english_list', 'zip_msa', 'female_list']
var_z_name_unord = []
...
# Optimal policy vars
VAR_X_NAME_ORD_OP = ['numhh_list','age', 'english_list', 'zip_msa', 'female_list']
VAR_X_NAME_UNORD_OP = []
...
result_train = pd.read_csv(DATPATH +'results_train.csv')
...
other_max_shares_values = [[1, 0.50]]
...
GEN_METHOD = 'policy tree'
pt_depth_tree1_values = [3] 
pt_depth_tree2_values= [1]
other_max_shares_values = [[1, 0.50]]#, [1, 0.38]] [1,0.49
other_costs_of_treat_mult = [1, 1.02]
    ...
    for pt_depth_tree_2 in pt_depth_tree2_values:
        for other_max_shares in other_max_shares_values:
            myoptp = OptimalPolicy(var_d_name=VAR_D_NAME,
                       ...
                       gen_method=GEN_METHOD,
                       other_max_shares = other_max_shares,
                       other_costs_of_treat_mult = [1, 1]
                       #pt_no_of_evalupoints = 1
                       )  #other_costs_of_treat_mult
            # Train the optimal tree
            alloc_train_df = myoptp.solve(data_train_pt, data_title='Training PT data')
